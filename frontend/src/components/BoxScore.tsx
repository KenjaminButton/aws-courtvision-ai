...
export function BoxScore({ boxscore, highlightIowa = true }: BoxScoreProps) {
  ...
  const isIowaHome = boxscore.homeTeam.name.toLowerCase().includes('iowa');
  const activeData = activeTeam === 'home' ? boxscore.homeTeam : boxscore.awayTeam;
  const isIowaActive = activeData.name.toLowerCase().includes('iowa');
  ...
  return (
    <div className="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
      ...
      <div className="flex border-b border-zinc-800">
        {teams.map(({ key, data }) => {
          const isIowa = data.name.toLowerCase().includes('iowa');
